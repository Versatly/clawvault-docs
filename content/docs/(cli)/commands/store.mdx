...
The `store` command is ClawVault's low-level storage primitive. It writes a document into the vault with explicit category, title, and content. For higher-level semantic storage, see [remember & capture](/docs/commands/remember-capture).
