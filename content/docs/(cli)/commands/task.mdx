...
Tasks are markdown files in `tasks/` with a slug-based filename (`<slug>.md`). Frontmatter tracks status and workflow metadata, including `status`, `priority`, `project`, `owner`, `blocked_by`, `due`, `tags`, `created`, `updated`, and `completed`.
