...
|------|-------------|---------|
| `-n, --limit <n>` | Max results | 5 |
| `--format <format>` | Output format (markdown/json) | markdown |
| `--recent` | Boost recent documents | true |
| `--include-observations` | Include observation memories in output | true |
| `--budget <number>` | Optional token budget for assembled context | — |
| `--profile <profile>` | Context profile (default/planning/incident/handoff/auto) | default |
