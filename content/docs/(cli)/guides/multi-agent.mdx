...
For isolated vaults, use `clawvault sync` to publish vault snapshots to a shared path:

```bash
# Agent Alpha exports a snapshot
clawvault sync /shared/agent-alpha-vault

# Agent Beta queries that snapshot directly
clawvault search "authentication architecture" -v /shared/agent-alpha-vault
clawvault context "auth decisions" -v /shared/agent-alpha-vault
```
