...
```bash
# Incremental pass every 15 minutes
*/15 * * * * clawvault observe

# Active-session sweep every 2 hours
0 */2 * * * clawvault observe --active
```
...
```
*/30 * * * * cd ~/vault && clawvault observe --active
0 3 * * * cd ~/vault && clawvault reindex
0 4 * * 0 cd ~/vault && clawvault link --all
0 5 * * 1 cd ~/vault && clawvault doctor
```
</Step>
<Step>
