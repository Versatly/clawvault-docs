...
| `completed` | string | ISO 8601 | Completion timestamp when status is `done` |
...
Every status change is appended to `ledger/transitions/YYYY-MM-DD.jsonl`. Each entry records:

```json
{
  "task_id": "ship-onboarding-flow",
  "agent_id": "clawdious",
  "from_status": "open",
...
Tasks integrate with Obsidian's Kanban plugin via the [`kanban`](/commands/kanban) command.
...
<Callout type="info">
For the full CLI reference of task flags, see the [`task` command page](/commands/task).
</Callout>
