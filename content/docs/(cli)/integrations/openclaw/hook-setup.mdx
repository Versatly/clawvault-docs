...
### Step 1: Install ClawVault

```bash
npm install -g clawvault
```

Or via ClawHub (installs the skill, but does **not** auto-register the hook):
...
Register the hook with OpenClaw by pointing it at the hook directory inside the clawvault package:
...
<Callout type="info">
The `clawhub install` command installs ClawVault as a skill but does **not** automatically register the OpenClaw hook. You must run `openclaw hooks install` separately.
</Callout>
...
You should see `clawvault` with status `✓ ready` in the output.

If the hook shows as disabled, enable it:
...
Alternatively, copy the hook handler directly into your OpenClaw hooks directory:

```bash
cp -r "$(npm root -g)/clawvault/hooks/clawvault" ~/.openclaw/hooks/clawvault
```
