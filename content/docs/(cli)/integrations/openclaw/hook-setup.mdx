...
The hook listens on three lifecycle events. Event names are normalized and matched flexibly -- separators like `.`, `:`, and `/` are treated interchangeably.
...
Fires on a weekly schedule. The hook:

1. Runs `clawvault reflect` to generate a weekly reflection summary
2. Stores the reflection in `ledger/reflections/YYYY-WNN.md`
3. Surfaces patterns, recurring themes, and notable trends from the week's observations

This provides automatic weekly introspection without manual intervention.

### session:start

Fires at the beginning of a new session, after the first user message is available. The hook:

1. Extracts the session key from the event context
2. Fetches a session recap using `clawvault session-recap` (recent conversation turns)
3. Extracts the initial user prompt and searches the vault for relevant memories using `clawvault context`
4. Injects both the recap and relevant memories as a system message

This gives the agent immediate access to prior conversation context and relevant vault knowledge.
