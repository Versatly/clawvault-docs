...
QMD integration is configured during `clawvault init` (via `--qmd` and `--qmd-collection` flags) and stored in `.clawvault.json`. Use `clawvault config list` to inspect runtime configuration, and edit `.clawvault.json` directly for qmd collection-specific values when needed.
