...
`doctor` returns a non-zero exit code on failure, which makes it CI-friendly without extra flags:

```bash
$ clawvault doctor
ClawVault found: /home/agent/.openclaw/workspace/memory
qmd available: true
graph index: healthy
hook compatibility: pass
```
...
<Callout type="info">
ClawVault commands already return meaningful exit codes in non-interactive environments, so they can be used directly in CI pipelines.
</Callout>
