...
### Added
- **WebDAV server** — `clawvault serve` now handles WebDAV protocol on `/webdav/` path prefix. Enables Obsidian mobile sync via the [Remotely Save](https://github.com/remotely-save/remotely-save) plugin over Tailscale. Supports GET, PUT, DELETE, MKCOL, PROPFIND, OPTIONS, HEAD, MOVE, and COPY operations.
...
### Added
- **Task tracking primitives** — `clawvault task` (add/list/update/done/show), `clawvault backlog` (add/list/promote), `clawvault blocked`. Tasks stored as markdown with frontmatter (status, owner, project, priority, blocked_by, due).
- **Canvas dashboard** — `clawvault canvas` generates an Obsidian JSON Canvas file with active tasks, blocked items, backlog, knowledge graph stats, vault activity, and data flow diagram. File nodes are clickable in Obsidian.
- **Vault activity stats** — canvas dashboard now shows observation count, reflection count, task completion rates, and document counts per category.
- **Tailscale networking** — `clawvault serve` (HTTP API on Tailscale IP), `clawvault peers` (manage vault peers), `clawvault net-search` (cross-vault search). Trust levels: read, read-write, full. Zero new dependencies.
